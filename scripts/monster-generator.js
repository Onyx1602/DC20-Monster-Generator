...
const ROLE_ADJUSTMENTS = {
  ambusher:     { movement: { ground: +1 }, attrib: { agility: +1 }, hp: -1, defenses: { pd: 1, ad: 0 } },
  berserker:    { movement: { ground: 0 },  attrib: { might: +1 }, hp: +1, defenses: { pd: -1, ad: 1 } },
  bruiser:      { movement: { ground: 0 },  attrib: { }, hp: +2, defenses: { pd: 1, ad: 1 } },
  caster:       { movement: { ground: -1 }, attrib: { intelligence: +1 }, hp: -2, defenses: { pd: 1, ad: 1 } },
  ...
  btn.on("click", () => {
    ...
    if (Math.floor(Math.random() * 20) === 0) {
      ...
      new Dialog({
        ...
        content: `<p>If you are enjoying this monster generator please consider supporting me here <a href="https://www.patreon.com/posts/135982278?collection=1652999" target="_blank">Premium Monster Generator</a>.<br><br>Creating these tools takes me a very long time and purchasing will help me make more content in the future.</p>`,
        buttons: {
          ok: { label: "OK" }
        },
        close: () => {
          new DC20MonsterGeneratorForm().render({ force: true });
        }
      }).render(true);
    } else {
      new DC20MonsterGeneratorForm().render({ force: true });
...
class DC20MonsterGeneratorForm extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
  static DEFAULT_OPTIONS = {
    tag: "div",
    classes: ["monster-generator-window"],
    position: { width: 540, height: "auto" },
    window: { title: "DC20 Monster Generator", resizable: false }
  };

  static PARTS = {
    form: { template: "modules/dc20-monster-generator/templates/monster-generator.html" }
  };

  static MONSTER_SUBTYPES = {
    "Aberrations": [
  ...
  async _prepareContext(options) {
    return {
      ...
      creatureSize: "medium",
      description: "",
      types: Object.keys(DC20MonsterGeneratorForm.MONSTER_SUBTYPES),
  ...
  _onRender(context, options) {
    ...
    const chooseBtn = html.querySelector("#chooseImageBtn");
    if (chooseBtn) {
      chooseBtn.addEventListener("click", () => {
        // Use namespaced FilePicker to avoid v13 deprecation warning
        const FilePickerClass = foundry.applications?.apps?.FilePicker?.implementation ?? FilePicker;
        if (FilePickerClass) {
          new FilePickerClass({
            type: "image",
            callback: path => {
              const input = html.querySelector("#customImage");
              const preview = html.querySelector("#customImagePreview");
    ...
    // 3. Subtype Population Logic
    const typeSelect = html.querySelector("#creatureType");
    const subtypeSelect = html.querySelector("#creatureSubtype");

    const populateSubtypes = () => {
      if (!typeSelect || !subtypeSelect) return;
      subtypeSelect.innerHTML = "";
      const typeValue = typeSelect.value;
      const subtypes = DC20MonsterGeneratorForm.MONSTER_SUBTYPES[typeValue];
    ...
    if (typeSelect) {
      typeSelect.addEventListener("change", populateSubtypes);
      // If re-rendering with a value, populate immediately
      if (typeSelect.value) populateSubtypes();
    }

    // 4. Video Playback
    const video = html.querySelector("#monster-generator-logo-video");
    if (video) {
      video.play().catch(() => { /* Auto-play might be blocked, ignore */ });
    }
  }

  async _onSubmit(event) {
    event.preventDefault();
    const FormDataExtendedCls = foundry.applications?.ux?.FormDataExtended || FormDataExtended;
    const formData = new FormDataExtendedCls(event.target).object;
  ...
  async _updateObject(event, formData) {
    ...
    // Helper to patch "duplicate(" to "foundry.utils.duplicate(" in item data (fix for v12+ deprecation in macros)
    const patchDuplicate = (itemData) => {
      if (!itemData) return itemData;
      try {
        let str = JSON.stringify(itemData);
        // Replace "duplicate(" with "foundry.utils.duplicate(" if not preceded by ".",
        // Using negative lookbehind for dot to avoid double patching
        if (str.includes("duplicate(")) {
          str = str.replace(/(?<!\.)\bduplicate\s*\(/g, "foundry.utils.duplicate(");
          return JSON.parse(str);
        }
      } catch (e) { console.error("Error patching duplicate:", e); }
      return itemData;
    };
    ...
    if (regenPack) {
      ...
      if (regenDoc) {
        ...
        if (!alreadyHas) {
          monsterData.features.push(patchDuplicate(regenDoc.toObject()));
        }
      }
    }
      ...
      for (const id of ids) {
        ...
        if (packObj) {
          ...
          if (doc) {
            ...
            if (!alreadyHas) {
              monsterData.features = monsterData.features || [];
              monsterData.features.push(patchDuplicate(doc.toObject()));
            }
          }
        }
      }
...
async function generateMonster(formData) {
  ...
  if (!formData._skipAbilities) {
    ...
    // Patch duplicate() in generated features too
...
if (typeof window !== "undefined") {
  window["get.dc20-monster-generator.regen-abilities"] = async function (macroItem) {
    // Helper to patch duplicate() in item data (fix for v12+ deprecation)
    const patchDuplicate = (itemData) => {
      if (!itemData) return itemData;
      try {
        let str = JSON.stringify(itemData);
    ...
    // Inject scoped styles once (Updated for V2 compatibility) - Moved before dialog
    try {
      const styleId = "dc20-regen-dialog-style";
      if (!document.getElementById(styleId)) {
        const style = document.createElement("style");
        style.id = styleId;
        style.textContent = `
          .dc20-regen-dialog .window-content { background: #2b2b2b !important; color: #ffffff; }
          .dc20-regen-dialog .dialog-buttons, .dc20-regen-dialog .form-footer { gap: 6px; }
          .dc20-regen-dialog button {
            background: #6d14cf !important;
            color: #ffffff !important;
            border-color: #6d14cf !important;
            filter: none !important;
            text-shadow: none !important;
          }
          /* Hover: lighter purple */
          .dc20-regen-dialog button:hover {
            background: #8b3af5 !important;
            color: #ffffff !important;
            border-color: #8b3af5 !important;
            box-shadow: none !important;
            outline: none !important;
          }
          /* Keep focus/active at base color to avoid false highlight */
          .dc20-regen-dialog button:focus,
          .dc20-regen-dialog button:active {
            background: #6d14cf !important;
            color: #ffffff !important;
            border-color: #6d14cf !important;
            box-shadow: none !important;
    ...
    const allAllowedItems = actor.items.filter(i => allowedTypes.includes(i.type));
    if (!allAllowedItems.length) {
      ui.notifications.warn("No weapons, techniques, spells, or features found on this actor.");
      return;
    }

    // Ask the user what to do (custom styled dialog, no default button highlight)
    let choice;
    if (foundry.applications?.api?.DialogV2) {
      // V13+ Support
      choice = await foundry.applications.api.DialogV2.wait({
        window: { title: "Regenerate Abilities" },
        classes: ["dc20-regen-dialog"],
        content: `<div class="dc20-regen-dialog-content" style="padding:6px;">Select an action:</div>`,
        buttons: [
          { action: "deleteAll", label: "Delete All Abilities", callback: () => "deleteAll" },
          { action: "regenAll", label: "Regenerate All Abilities", callback: () => "regenAll" },
          { action: "regenOne", label: "Regenerate Specific Ability", callback: () => "regenOne" }
        ],
        close: () => null
      });
    } else {
      ...
      choice = await new Promise(resolve => {
        new Dialog({
          title: "Regenerate Abilities",
          content: `<div class="dc20-regen-dialog-content" style="padding:6px;">Select an action:</div>`,
          buttons: {
            deleteAll: { label: "Delete All Abilities", callback: () => resolve("deleteAll") },
            regenAll:  { label: "Regenerate All Abilities", callback: () => resolve("regenAll") },
            regenOne:  { label: "Regenerate Specific Ability", callback: () => resolve("regenOne") }
          },
          close: () => resolve(null)
        }, { classes: ["dc20-regen-dialog"] }).render(true);
      });
    }
    if (!choice) return;

    const protectedNames = new Set(["Buff/Debuff", "Regenerate Abilities"]);
    ...
    function getItemCompendiumId(it) {
      if (it.flags?.core?.sourceId) return it.flags.core.sourceId;
      if (typeof it._id === "string" && it._id.startsWith("Compendium.")) return it._id;
      if (it.system?.sourceId) return it.system.sourceId;
      return null;
    }

    // If two roles, randomly pick one for this regeneration
    ...
    // Function to fetch a new non-duplicate compendium doc for a given item
    ...
    const existingSourceIds = new Set(
      actor.items
        .map(i => i.flags?.core?.sourceId || (typeof i._id === "string" && i._id.startsWith("Compendium.") ? i._id : null))
        .filter(Boolean)
    );
    ...
    async function regenerateOneItem(selectedItem) {
      ...
      while (attempts < MAX_ATTEMPTS) {
        attempts++;
        const result = getRandomAbility({ type: creatureType, role: chosenRole, avoidId: itemCompendiumId });
      ...
      if (!doc) return { replaced: false };

      // Scale damage based on actor level
      const DAMAGE_BY_LEVEL = {
        "-1": "1", "0": "1", "1": "2", "2": "2", "3": "3", "4": "4", "5": "5", "6": "5",
        "7": "6", "8": "6", "9": "7", "10": "7", "11": "8", "12": "8", "13": "9", "14": "10",
        "15": "11", "16": "11", "17": "12", "18": "12", "19": "13", "20": "13"
      ...
      const tableDamage = Object.prototype.hasOwnProperty.call(DAMAGE_BY_LEVEL, String(lvl)) ? DAMAGE_BY_LEVEL[String(lvl)] : "2";
      let docObj = doc.toObject();
      
      // Patch duplicate() calls in the item data (e.g. macros) before creation
      docObj = patchDuplicate(docObj);
      ...
      if (docObj.system && typeof docObj.system.description === "string") {
        ...
        docObj.system.description = docObj.system.description.replace(/deals\s+\d+\s*damage/gi, `deals ${tableDamage} damage`);
        docObj.system.description = docObj.system.description.replace(/(take(?:s)?\s+)\d+(\s*damage)/gi, (m, p1, p2) => `${p1}${tableDamage}${p2}`);
      }

      await selectedItem.delete();
      const [created] = await actor.createEmbeddedDocuments("Item", [docObj]);

      // Update duplicate-avoid sets
      existingNames.add(created.name);
      const sid = created.flags?.core?.sourceId || (typeof created._id === "string" && created._id.startsWith("Compendium.") ? created._id : null);
      if (sid) existingSourceIds.add(sid);

      return { replaced: true, poolType, newName: created?.name };
    }

    // Branch by choice
    if (choice === "deleteAll") {
      const toDelete = actor.items.filter(i => allowedTypes.includes(i.type) && !protectedNames.has(i.name));
      if (!toDelete.length) {
        ui.notifications.info("No abilities to delete.");
        return;
      }
      await actor.deleteEmbeddedDocuments("Item", toDelete.map(i => i.id));
      ui.notifications.info(`Deleted ${toDelete.length} abilities (protected items kept).`);
      return;
    }

    if (choice === "regenAll") {
      const regenItems = actor.items.filter(i => allowedTypes.includes(i.type) && !protectedNames.has(i.name));
      if (!regenItems.length) {
        ui.notifications.info("No abilities to regenerate.");
        return;
      }
      let replacedCount = 0;
      for (const it of regenItems) {
        const res = await regenerateOneItem(it);
        if (res.replaced) replacedCount++;
      }
      ui.notifications.info(`Regenerated ${replacedCount} abilities (protected items kept).`);
      return;
    }

    // Regenerate Specific Ability (original flow)
    // Build options for popup (show type in label)
    const options = Object.fromEntries(allAllowedItems.map(i => [i.id, `[${i.type}] ${i.name}`]));
    ...
    const res = await regenerateOneItem(selectedItem);
    if (!res.replaced) {
      ui.notifications.warn("No unique ability found for this actor's type or role.");
      return;
    }
    ui.notifications.info(`${selectedItem.name} was replaced with [${res.newName}]${res.poolType ? ` from the ${res.poolType} pool` : ""}.`);
  };
}
